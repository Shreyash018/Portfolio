...
function LandingPage() {
  ...
  return (
    <div data-scroll data-scroll-section data-scroll-speed="-.4" className='w-full h-screen bg-zinc-900 pt-[2vw] md:pt-[0.5vw]'>
      <div className='textstructure mt-[12vw] md:mt-[9vw] px-[5vw]'>
              ...
              <div className='w-fit flex'>
                {index === 1 && (
                  <motion.div
                    ...
                    className='mr-[1.5vw] w-[8vw] rounded-md h-[10vw] md:h-[5.3vw] relative -bottom-[2vw] md:-bottom-[1vw] bg-red-500'
                  />
                )}
                <h1 className='uppercase text-5xl md:text-9xl leading-[7vw] md:leading-[7.2vw] tracking-tighter font-["Founders_Grotesk"] font-semibold'>{items}</h1>
              </div>
            </div>
          );
        })}
      ...
      <div className="border-t-[1px] border-zinc-700 mt-20 flex flex-col md:flex-row justify-between items-center py-5 px-6 md:px-20">
        {["For public and private companies", "From the first pitch to IPO"].map((items, index) => (
          <p key={index} className='text-md md:text-md font-light tracking-tighter leading-none mt-2 md:mt-0'>{items}</p>
        ))}
        <div className='start flex items-center gap-3 md:gap-5 mt-4 md:mt-0'>
          <div className='group flex gap-3'>
            <button
              className='px-4 md:px-5 py-2 border-[2px] border-zinc-400 rounded-full font-light text-md md:text-md uppercase group-hover:bg-white group-hover:text-black'
              onClick={handleButtonClick}
            >
              Start The Project
            </button>
            <div className='w-8 md:w-10 h-8 md:h-10 flex items-center justify-center border-[2px] border-zinc-400 rounded-full group-hover:bg-white group-hover:text-black'>
