...
function Featured() {
  ...
  return (
    <div className='w-full py-10 md:py-20'>
      <div className='w-full px-6 md:px-20 flex flex-col md:flex-row items-start md:items-center border-b-[1px] pb-10 md:pb-20 border-zinc-600'>
        <h1 className='text-5xl md:text-8xl text-gray-500 font-["Neue_Montreal"]'>/</h1>
        <h1 className='text-5xl md:text-8xl font-["Neue_Montreal"] tracking-tight'>Featured Projects</h1>
        <h1 className='text-5xl md:text-8xl text-gray-500 font-["Neue_Montreal"]'>.</h1>
      </div>

      <div className='px-6 md:px-20'>
        <div className='cards w-full flex flex-col md:flex-row gap-10 mt-10 mb-10 md:mb-20'>
          <motion.div
            onHoverStart={() => handleHover(0)}
            onHoverEnd={() => handleHoverEND(0)}
            className='cardcontainer relative w-full md:w-1/2 h-[50vh] md:h-[75vh]'
          >
            <h1 className='absolute flex overflow-hidden text-white left-full -translate-x-1/2 top-1/2 -translate-y-1/2 z-[9] font-["Founders_Grotesk"] font-semibold leading-none tracking-tighter text-6xl md:text-8xl'>
              {"Mini Waggering App".split("").map((item, index) => (
                <motion.span
                  key={index}
                  initial={{ y: "100%" }}
                  animate={cards[0]}
            ...
            <a href='https://github.com/Shreyash018/mini-wagering' target='_blank' rel='noopener noreferrer'>
              <img
                className="w-full h-full rounded-xl bg-cover transition-transform transform hover:scale-90"
                src={pro1}
                alt=''
              />
            </a>
            ...
            <div className='links flex gap-3 mt-5'>
              {["React Native", "Tailwind CSS", "Firebase", "Expo CLI"].map((item, index) => (
                <a key={index} className="px-3 py-1 border-[2px] border-zinc-400 rounded-full text-xs md:text-base">{item}</a>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className='cardcontainer relative w-full md:w-1/2 h-[50vh] md:h-[75vh]'
          >
            <h1 className='absolute flex overflow-hidden text-white right-full translate-x-1/2 top-1/2 -translate-y-1/2 z-[9] font-["Founders_Grotesk"] font-semibold leading-none tracking-tighter text-6xl md:text-8xl'>
              {"DeBank Crypto Token".split("").map((item, index) => (
                <motion.span
                  key={index}
                  initial={{ y: "100%" }}
                  animate={cards[1]}
        ...
        <div className='cards w-full flex flex-col md:flex-row gap-10 mt-10'>
          <div className='cardcontainer relative w-full md:w-1/2 h-[50vh] md:h-[75vh]'>
            <h1 className='absolute flex overflow-hidden text-[#CDEA68] left-full -translate-x-1/2 top-1/2 -translate-y-1/2 z-[9] font-["Founders_Grotesk"] font-semibold leading-none tracking-tighter text-6xl md:text-8xl'>
              {"FYDE".split("").map((item, index) => (
                <span key={index} className='inline-block translate-y-full'>{item}</span>
              ))}
            </h1>
            <img className="w-full h-full rounded-xl bg-cover" src={pro1} alt='' />
          </div>

          <div className='cardcontainer relative w-full md:w-1/2 h-[50vh] md:h-[75vh]'>
            <h1 className='absolute flex overflow-hidden text-[#CDEA68] right-full translate-x-1/2 top-1/2 -translate-y-1/2 z-[9] font-["Founders_Grotesk"] font-semibold leading-none tracking-tighter text-6xl md:text-8xl'>
              {"VISE".split("").map((item, index) => (
                <span key={index} className='inline-block translate-y-full'>{item}</span>
              ))}
            </h1>
            <img className="w-full h-full rounded-xl bg-cover" src={pro2} alt='' />
          </div>
        </div>
      </div>
    </div>
